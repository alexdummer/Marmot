...
void testComputeYourselfExplicitMatchesImplicitPlaneStrainQuad8()
{
  constexpr int nDim    = 2;
  ...
  Eigen::VectorXd q    = Eigen::VectorXd::Zero( nDof );
  Eigen::VectorXd dQ( nDof );
  dQ << 1.0e-4, -2.0e-4, 0.5e-4, 1.2e-4, -1.1e-4, 0.9e-4, 2.0e-4, -0.3e-4, 0.7e-4, -1.3e-4, 1.4e-4, 0.4e-4, -0.6e-4,
    1.1e-4, -0.8e-4, 0.2e-4;
  ...
  throwExceptionOnFailure( pExplicit.isApprox( pImplicit, 1e-10 ),
                           "Explicit internal force does not match implicit result for quad8." );
  throwExceptionOnFailure( checkIfEqual( pNewDtExp, 1.0, 1e-12 ), "Unexpected pNewDt in explicit quad8 computation." );

  for ( size_t i = 0; i < elementImplicit->qps.size(); i++ ) {
    const auto& implicitState = elementImplicit->qps[i].managedStateVars;
    const auto& explicitState = elementExplicit->qps[i].managedStateVars;
    throwExceptionOnFailure( implicitState->stress.isApprox( explicitState->stress, 1e-10 ),
                             "Stress mismatch between explicit and implicit update for quad8." );
    throwExceptionOnFailure( implicitState->strain.isApprox( explicitState->strain, 1e-10 ),
                             "Strain mismatch between explicit and implicit update for quad8." );
  }
}
...
int main()
{
  auto tests = std::vector< std::function< void() > >{ testInstantiationAndBasicProperties,
                                                       ...
                                                       testComputeYourselfExplicitMatchesImplicitPlaneStrainQuad8,
